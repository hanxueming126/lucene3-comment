...
public final class SegmentInfo {
  ...
  /**•段名，所有属于同一个段的文件都有以段名作为文件名，第一个段的段名为"_0"，第二个段的段名为"_1"*/
  ...
  /**•.del文件的版本号.*/
  private long delGen;                            // current generation of del file; NO if there are no deletes; CHECK_DIR if it's a pre-2.1 segment
                                                  // (and we must check filesystem); YES or higher if
                                                  // there are deletes at generation N
  /**如果每个域有自己的标准化因子文件(hasSingleNormfile=true)，则此数组描述了每个标准化因子文件的版本号，也即.fN的N*/
  private long[] normGen;                         // current generation of each field's norm file.If this array is null, for lockLess this means no 
  ...
  private byte isCompoundFile;                    // NO if it is not; YES if it is; CHECK_DIR if it's pre-2.1 (ie, must check file system to see
                                                  // if <name>.cfs and <name>.nrm exist)         
 /** 如果HasSingleNormFile为1，则所有的标准化因子都是存在.nrm文件中，否则每个域都有自己的标准化因子文件.fN*/
  private boolean hasSingleNormFile;              // true if this segment maintains norms in a single file;false otherwise
  ...
  /**•如果DocStoreOffset为-1，则此段单独存储自己的域和词向量，
    *从存储文件上来看，如果此段段名为XXX，则此段有自己的XXX.fdt，XXX.fdx，XXX.tvf，XXX.tvd，XXX.tvx文件。
    *•如果DocStoreOffset不为-1，则DocStoreSegment保存了共享的段的名字，
    *比如为YYY，DocStoreOffset则为此段的域及词向量信息在共享段中的偏移量。
    *则此段没有自己的XXX.fdt，XXX.fdx，XXX.tvf，XXX.tvd，XXX.tvx文件，
    *而是将信息存放在共享段的YYY.fdt，YYY.fdx，YYY.tvf，YYY.tvd，YYY.tvx文件中。*/
  private int docStoreOffset;                     // if this segment shares stored fields & vectors, this offset is where in that file this segment's docs begin
  /**共享tvx/tvd/tvf/fdt/fdx文件的段的名字：如XXX.fdt文件中的XXX*/
  private String docStoreSegment;                 // name used to derive fields/vectors file we share with other segments
  ...
  /**
   * 按指定格式写入段信息（seg01） Save this segment's info.
   */
  void write(IndexOutput output)
    throws IOException {
    output.writeString(name);//•段名，所有属于同一个段的文件都有以段名作为文件名，第一个段的段名为"_0"，第二个段的段名为"_1"
    output.writeInt(docCount);//•此段中包含的文档数
    output.writeLong(delGen);//•.del文件的版本号
    output.writeInt(docStoreOffset);// •如果DocStoreOffset为-1，则此段单独存储自己的域和词向量，从存储文件上来看，如果此段段名为XXX，则此段有自己的XXX.fdt，XXX.fdx，XXX.tvf，XXX.tvd，XXX.tvx文件。
    if (docStoreOffset != -1) {//•如果DocStoreOffset不为-1，则DocStoreSegment保存了共享的段的名字，比如为YYY，DocStoreOffset则为此段的域及词向量信息在共享段中的偏移量。则此段没有自己的XXX.fdt，XXX.fdx，XXX.tvf，XXX.tvd，XXX.tvx文件，而是将信息存放在共享段的YYY.fdt，YYY.fdx，YYY.tvf，YYY.tvd，YYY.tvx文件中。
      output.writeString(docStoreSegment);//DocStoreSegment保存了共享的段的名字
      output.writeByte((byte) (docStoreIsCompoundFile ? 1:0));
    }

    output.writeByte((byte) (hasSingleNormFile ? 1:0));//•如果HasSingleNormFile为1，则所有的标准化因子都是存在.nrm文件中，否则每个域都有自己的标准化因子文件.fN 
    if (normGen == null) {
      output.writeInt(NO);//•NumField:•域的数量
    } else {
      output.writeInt(normGen.length);//•NumField:•域的数量
      for(int j = 0; j < normGen.length; j++) {
        output.writeLong(normGen[j]);//•normGen:如果每个域有自己的标准化因子文件，则此数组描述了每个标准化因子文件的版本号，也即.fN的N
      }
    }
    output.writeByte(isCompoundFile);//•是否保存为复合文件，也即把同一个段中的文件按照一定格式，保存在一个文件当中，这样可以减少每次打开文件的个数。 •是否为复合文件，由接口IndexWriter.setUseCompoundFile(boolean)设定。  
    output.writeInt(delCount);//•记录了此段中删除的文档的数目
    output.writeByte((byte) (hasProx ? 1:0));//•如果至少有一个段omitTf为false，也即词频(term freqency)需要被保存，则HasProx为1，否则为0。
    output.writeStringStringMap(diagnostics);//•调试信息
  }
