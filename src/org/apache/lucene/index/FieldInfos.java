...
final class FieldInfos {
  ...
  /**
   * 将fieldInfos中存储的域元数据信息写入磁盘(.fnm文件)
   */
...
public void write(IndexOutput output) throws IOException {
  ...
  private void read(IndexInput input, String fileName) throws IOException {
    int firstInt = input.readVInt();//•是fnm文件的版本号，对于Lucene 2.9为-2
    ...
    if (format == FORMAT_PRE) {
      ...
      size = input.readVInt(); //read in the size:•域的数目
    }

    for (int i = 0; i < size; i++) {//遍历所有域：封装域中的信息到FieldInfo对象（域号为数组下标）
      String name = StringHelper.intern(input.readString());//•FieldName：域名，如"title"，"modified"，"content"等。 
      byte bits = input.readByte();//•FieldBits:8bits，一系列标志位，表明对此域的索引方式
      boolean isIndexed = (bits & IS_INDEXED) != 0;//•最低位：1表示此域被索引，0则不被索引。所谓被索引，也即放到倒排表中去。
      boolean storeTermVector = (bits & STORE_TERMVECTOR) != 0;//•倒数第二位：1表示保存词向量，0为不保存词向量。
      boolean storePositionsWithTermVector = (bits & STORE_POSITIONS_WITH_TERMVECTOR) != 0;//•倒数第三位：1表示在词向量中保存位置信息。 
      boolean storeOffsetWithTermVector = (bits & STORE_OFFSET_WITH_TERMVECTOR) != 0;//•倒数第四位：1表示在词向量中保存偏移量信息。
      boolean omitNorms = (bits & OMIT_NORMS) != 0;//•倒数第五位：1表示不保存标准化因子 
      boolean storePayloads = (bits & STORE_PAYLOADS) != 0;//•倒数第六位：是否保存payload 
