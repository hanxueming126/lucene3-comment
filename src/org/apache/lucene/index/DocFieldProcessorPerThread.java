...
final class DocFieldProcessorPerThread extends DocConsumerPerThread {
  ...
  /**创建一个哈希表来方便更快查找域对应的处理对象：链地址法存储（如果两个不同的域映射到同一数组元素，则采用链表存储）*/
  ...
  /** 
   * 回收fieldHash（本轮flush操作中没用到的field），以便用于下一轮的索引，为提高效率，索引链中的对象是被复用的。<br>
...
//  存储域：清理
    ...
    for(int i=0;i<numDocFields;i++) {
      ...
      while(fp != null && !fp.fieldInfo.name.equals(fieldName))//查找复用域处理对象的域链表中域名相同的域，如果存在则不必再创建
        fp = fp.next;
      ...
      if (fp == null) {
        ...
        fp.fieldInfo.update(field.isIndexed(), field.isTermVectorStored(),
                            field.isStorePositionWithTermVector(), field.isStoreOffsetWithTermVector(),
                            field.getOmitNorms(), false, field.getOmitTermFreqAndPositions());
      //当this中的版本（fieldGen）和fp中的版本（fp.lastGen）不一致时：同步fp到fields
      ...
      if (field.isStored()) {//处理存储域：如果是存储域的话，用StoredFieldsWriterPerThread将其写到磁盘索引中
    ...
    for(int i=0;i<fieldCount;i++)//处理索引域
...
/**
 * 将数组array按照fieldInfo.name从小到大排序
 * @param array
 * @param lo
 * @param hi
 */
