...
final class FieldsWriter
{
  ...
  static final int FORMAT_CURRENT = FORMAT_LUCENE_3_0_NO_COMPRESSED_FIELDS;
    ...
    /**保存field到磁盘域数据文件(fdt：真正保存存储域(stored field)的信息)*/
    ...
    /**
     * 保存field到域数据文件(fdt)：内存级fieldsStream=RAMOutputStream/磁盘级fieldsStream=SimpleFSIndexOutput文件/db等
     * @param fi
     * @param field
     * @throws IOException
     */
    final void writeField(FieldInfo fi, Fieldable field) throws IOException {
      fieldsStream.writeVInt(fi.number);//域号：C-ntbcv
      byte bits = 0;
      if (field.isTokenized())//此域是否分词
        bits |= FieldsWriter.FIELD_IS_TOKENIZED;
      if (field.isBinary())//此域是保存字符串数据还是二进制数据
        bits |= FieldsWriter.FIELD_IS_BINARY;
                
      fieldsStream.writeByte(bits);//写域的属性位
