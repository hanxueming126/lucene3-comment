...
final class TermVectorsTermsWriter extends TermsHashConsumer {
  ...
  synchronized void closeDocStore(final SegmentWriteState state) throws IOException {
    if (tvx != null) {
      ...
      fill(state.numDocsInStore - docWriter.getDocStoreOffset());
      tvx.close();//关闭tvx, tvf, tvd 文件的写入流
      tvf.close();
      tvd.close();
      tvx = null;
      ...
      //从DocumentsWriter 的成员变量openFiles中删除，未来可能被IndexFileDeleter删除:
  ...
  /** 
   * 为不保存词向量的文档在tvd文件中写入零。即便不保存词向量，在tvx, tvd中也保留一个位置<br>
