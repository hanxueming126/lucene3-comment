...
public abstract class LogMergePolicy extends MergePolicy {
  ...
  /**当大小几乎相当的段的数量达到此值的时候，开始合并。*/
  private int mergeFactor = DEFAULT_MERGE_FACTOR;
  /**所有大小小于此值的段，都被认为是大小几乎相当，一同参与合并。*/
  long minMergeSize;
  /**当一个段的大小大于此值的时候，就不再参与合并。*/
  long maxMergeSize;
  /**当一个段包含的文档数大于此值的时候，就不再参与合并。*/
  ...
  public MergeSpecification findMerges(SegmentInfos infos) throws IOException {
    ...
    /*(1) 生成levels 数组，每个段一项。然后根据每个段的大小，计算每个项的值，
     * levels[i]和段的大小的关系为Math.log(size)/Math.log(mergeFactor)*/
    for(int i=0;i<numSegments;i++) {
      final SegmentInfo info = infos.info(i);
      long size = size(info);
    ...
    /*(2) 由于段基本是按照由大到小排列的，而且合并段应该大小差不多的段中进行。我们把大小差不
    多的段称为属于同一阶梯，因而此处从第一个段开始找属于相同阶梯的段，如果属于此阶梯的段数量
    达到mergeFactor 个，则生成合并任务，否则继续向后寻找下一阶梯。*/
  ...
  //计算最低阶梯值，所有小于此值的都属于最低阶梯
    ...
    while(start < numSegments) {
      ...
      float levelBottom;
     /* 计算出此阶梯的谷值levelBottom。*/
      if (maxLevel < levelFloor)//如果峰值maxLevel小于最低阶梯值levelFloor， 则所有此阶梯的段都属于最低阶梯。
        // All remaining segments fall into the min level
        levelBottom = -1.0F;
      else {//如果峰值大于等于最低阶梯值，谷值=最大值减去0.75，（levelBottom，maxLevel）之间的都属于此阶梯
        levelBottom = (float) (maxLevel - LEVEL_LOG_SPAN);

        // Force a boundary at the level floor
        //谷值小于最低阶梯值，则设置谷值为 最低阶梯值，以保证所有小于最低阶梯值的段都属于最低阶梯。
