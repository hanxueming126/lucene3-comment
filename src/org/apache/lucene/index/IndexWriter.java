...
public class IndexWriter implements Closeable {
  ...
  /**影响打分的标准化因子(normalization factor)部分，
    *对文档的打分分两个部分，一部分是索引阶段计算的，
    *与查询语句无关，一部分是搜索  阶段计算的，与查询语句相关。
    **/
  private Similarity similarity = Similarity.getDefault(); // how to normalize

  private volatile long changeCount; // increments every time a change is completed
  private long lastCommitChangeCount; // last changeCount that was committed
//start:为保持索引完整性，一致性和事务性
  /**当IndexWriter对索引进行了添加，删除文档操作后，可以调用commit将修改提交到文件中去，
    *也可以调用rollback取消从上次commit到此时的修改。**/
  private SegmentInfos rollbackSegmentInfos;      // segmentInfos we will fallback to if the commit fails
  private HashMap<SegmentInfo,Integer> rollbackSegments;

  volatile SegmentInfos pendingCommit;            // set when a commit is pending (after prepareCommit() & before commit())
  volatile long pendingCommitChangeCount;
  /**此段信息主要用于将其他的索引文件夹合并到此索引文件夹的时候，
    *为防止合并到一半出错可回滚所保存的原来的段信息。*/
  private SegmentInfos localRollbackSegmentInfos;      // segmentInfos we will fallback to if the commit fails
  private int localFlushedDocCount;               // saved docWriter.getFlushedDocCount during local transaction
//end:为保持索引完整性，一致性和事务性
  /**保存段信息，大家会发现，和segments_N中的信息几乎一一对应。*/
  private SegmentInfos segmentInfos = new SegmentInfos();       // the segments

  private DocumentsWriter docWriter;
  /**此对象不是用来删除文档的，而是用来管理索引文件的：在对文档的添加，删除，对段的合并的处理过程中，
    *会生成很多新的文件，并需要删除老的文件，因而需要管理。然而要被删除的文件又可能在被用，
    *因而要保存一个引用计数，仅仅当引用计数为零的时候，才执行删除。*/
  private IndexFileDeleter deleter;
  /**保存正在最优化(optimize)的段信息。
   * 当调用 optimize的时候，当前所有的段信息加入此Set，此后新生成的段并不参与此次最优化。
   **/
...
//start:用于合并段，在合并段的文章中将详细描述
  ...
  /*段合并器:用于创建一个线程来进行背后的段合并(当达到MergeFactor数量时，ConcurrentMergeScheduler$MergeThread.run())*/
...
//end:用于合并段，在合并段的文章中将详细描述
  ...
  private void closeInternal(boolean waitForMerges) throws CorruptIndexException, IOException {
    ...
    try {
      ...
      if (!hitOOM) {
        flush(waitForMerges, true, true);//将索引信息由内存写入磁盘
      }

      if (waitForMerges)
        // Give merge scheduler last chance to run, in case
        // any pending merges are waiting:
        mergeScheduler.merge(this);//进行段合并
...
/**
 * 主要负责找到可以合并的段，并生产段合并任务对象，并向段合并器注册这个任务。
 * @param maxNumSegmentsOptimize
 * @param optimize
...
private synchronized void updatePendingMerges(int maxNumSegmentsOptimize, boolean optimize)
    ...
    if (spec != null) {
      final int numMerges = spec.merges.size();
      for(int i=0;i<numMerges;i++)
        //向段合并器注册合并任务，将任务加到pendingMerges 中：
        registerMerge(spec.merges.get(i));
    }
  }
  ...
  /**
   * 将索引信息由内存写入磁盘Flush all in-memory buffered udpates (adds and deletes)
   * to the Directory.
  ...
  protected final void flush(boolean triggerMerge, boolean flushDocStores, boolean flushDeletes) throws CorruptIndexException, IOException {
    ...
    if (doFlush(flushDocStores, flushDeletes) && triggerMerge)
      maybeMerge();//当缓存写入硬盘，形成了新的段后，就有可能触发一次段合并，所以调用maybeMerge()
  }
  ...
  /**
   * 将索引写入磁盘包括以下几个过程：<br/>
   *  1、得到要写入的段名：String segment = docWriter.getSegment();<br/>
   *  2、 DocumentsWriter 将缓存的信息写入段：docWriter.flush(flushDocStores);<br/>
   *  3、生成新的段信息对象：newSegment = new SegmentInfo(segment, flushedDocCount,directory, false, true, docStoreOffset, docStoreSegment, docStoreIsCompoundFile,docWriter.hasProx());<br/>
   *  4、准备删除文档：docWriter.pushDeletes();<br/>
   *  5、生成cfs 段：docWriter.createCompoundFile(segment);<br/>
   *  6、删除文档：applyDeletes();<br/>
   */
private synchronized final boolean doFlushInternal(boolean flushDocStores, boolean flushDeletes) throws CorruptIndexException, IOException {
    ...
    try {
      ...
      String docStoreSegment = docWriter.getDocStoreSegment();//要写入的目标段名称
      ...
      int docStoreOffset = docWriter.getDocStoreOffset();//存储域在目标段中的偏移量，为-1时单独存储
