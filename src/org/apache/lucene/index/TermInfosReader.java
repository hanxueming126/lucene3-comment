    ...
    try {
      ...
      if (indexDivisor != -1) {
        ...
        try {
          ...
          for (int i = 0; indexEnum.next(); i++) {//使用3个数组对象封装tii文件中的TermIndex信息:查看索引文件格式即可知晓
            indexTerms[i] = indexEnum.term();
            indexInfos[i] = indexEnum.termInfo();//有多少篇文档包含此词(DocFreq)，此词的倒排表在frq，prx中的偏移量(FreqDelta, ProxDelta)，此词的倒排表的跳跃表在frq中的偏移量(SkipDelta)，这里之所以用Delta，是应用差值规则。 
