...
final class TermInfosWriter {
  ...
  /**表示此对象是用来写tii文件的还是用来写tis文件*/
  private boolean isIndex;
  private byte[] lastTermBytes = new byte[10];
  private int lastTermBytesLength = 0;
  private int lastFieldNumber = -1;
  /**isIndex=false则，它是用来写tis文件的，它的other指向的是用来写tii文件的TermInfosWriter对象，<br>
   * isIndex=true 则，它是用来写tii文件的，它的other指向的是用来写tis文件的TermInfosWriter对象*/
  ...
  /** 
   * 将词典(terminfo)写入tii,tis 文件：<br>
   * -tii 文件是tis 文件的类似跳表的东西，是在tis文件中每隔indexInterval个词提取出一个词放在tii文件中，以便很快的查找到词。<br>
   * -因而TermInfosWriter类型中有一个成员变量other也是TermInfosWriter类型的，还有一个成员变量isIndex来表示此对象是用来写tii文件的还是用来写tis文件的。<br>
   * -如果一个TermInfosWriter对象的isIndex=false则，它是用来写tis文件的，它的other指向的是用来写tii文件的TermInfosWriter对象<br>
   * -如果一个TermInfosWriter对象的isIndex=true则，它是用来写tii文件的，它的other指向的是用来写tis文件的TermInfosWriter对象<br>
   * 
  ...
  void add(int fieldNumber, byte[] termBytes, int termBytesLength, TermInfo ti)
    ...
    //如果词的总数是indexInterval 的倍数，则应该写入tii 文件:
